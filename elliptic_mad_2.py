...
def elliptic_contructer(lidar_vecs: np.ndarray, lidar_vec_dots: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
   ...
   # k = 1/2
    ...
    R_dot[:, 1, 0] = s_dots[:, 1] / s_norm - s_coeff * s[:, 1]
    R_dot[:, 1, 1] = s_dots[:, 0] / s_norm - s_coeff * s[:, 0]
    ...
    # More calculations
    h = np.einsum('ni,nij,nj->n', r, P_1, r) - k**2 * np.einsum('ni,ni->n', s, s) - epsilon
    h_dot = 2 * np.einsum('ni,nij,nj->n', r_dots, P_1, r) + 2 * np.einsum('ni,nij,nj->n', r, P_2, r) - 2 * k**2 * np.einsum('ni,ni->n', s, s_dots)

    # Print minimum h
    print("Minimum h:", np.min(h))

    psi_1 = 2 * np.einsum('ni,nij,nj->n', r_dots, P_1, r) + 2 * np.einsum('ni,nij,nj->n', r, P_2, r) - 2 * k**2 * np.einsum('ni,ni->n', s, s_dots) + alpha_1 * h
    print("Psi1 min:", np.min(psi_1))

    Lg_psi_1 = -2 * np.einsum('ni,nij->nj', r, P_1)
    Lf_psi_1 = 2 * np.einsum('ni,nij,nj->n', r, R_dot_dot @ Lambda @ R.transpose(0, 2, 1), r) + 4 * np.einsum('ni,nij,nj->n', r_dots, P_2, r_dots) + 4 * np.einsum('ni,nij,nj->n', r_dots, P_2.transpose(0, 2, 1), r) + 2 * np.einsum('ni,nij,nj->n', r_dots, P_1, r_dots) + 2 * np.einsum('ni,nij,nj->n', r, R_dot @ Lambda @ R_dot.transpose(0, 2, 1), r) - 2 * k**2 * np.einsum('ni,ni->n', s_dots, s_dots) + alpha_1 * h_dot
